...
bool OLED_init(void)
{
    uint8_t oled32_initbuf[] = {0x00,
                                ...
                                0xd5, // cmd: set display clock 设置显示时钟分频因子/振荡器频率。0x80用于设置默认时钟分频因子。更改此值可能会影响显示器的性能和稳定性。
                                0x80,
                                0xa8, // cmd: set multiplex ratio  设置多路复用比率。0x1f（31）对应于屏幕的高度减去1。这里设置了1/32偏置
                                0x1f, // HEIGHT - 1 -> 31
                                0xd3, // cmd: Set display offset, 设置显示偏移量。通常用于微调显示的位置 0x08 0xF8
                                0x05,
                                0x40, // cmd: Set Display Start Line 设置显示起始行为第一行。
                                0x8d,
                                0x14, // cmd: Set Higher Column Start Address for Page Addressing Mode 设置RAM内容的显示起始行（在Page Addressing Mode下的列的高位起始地址）。这个值的具体含义会根据屏幕的布局和驱动器而有所不同
                                0xa1, // 设置段重映射。A1/A0用于设置列扫描方向的反转。
                                0xc8, // cmd: Set COM Output Scan Direction C0/C8 设置COM输出扫描方向。C8/C0用于设置行扫描方向的反转 
                                0xda, // cmd: Set COM Pins Hardware Configuration 设置COM引脚硬件配置。这里设置了COM引脚配置为双引脚（bit [5:4] = 0b10）
                                0x02, //
                                0x81, // cmd: Set Contrast control 设置对比度控制。对比度的值可以在0x00至0xFF之间调节
                                0x7f,
                                0xd9, // cmd: Set Pre-Charge Period 设置预充电周期。这个周期对于OLED的寿命和显示效果有一定的影响
                                0xf1,
                                0xdb, // comd: Vcom regulator output 设置Vcomh电压输出。这个值通常是用于调节显示的负偏压
                                0x40,
                                0xa4,  // cmd: display on ram contents 设置RAM内容显示开启。如果设置为0xA4，将显示存储器的内容；如果设置为0xA5，将忽略存储器内容，全屏点亮
                                0xa6,  // cmd: set normal 设置正常显示模式。如果设置为0xA6，显示反转；如果设置为0xA7，显示正常
                                0xaf}; // cmd: display on
    uint8_t uc[4];
